#Read user input
numbers = list(map(int, input().split(" ")))
n = int(input())

#Logic
for i in range(n):
    numbers.remove(min(numbers))

numbers = list(map(str, numbers))
result = ", ".join(numbers)

#Print result
print(result)


# По-бърз метод на решаване относно производителност:
"""
numbers.sort()
numbers = numbers[n:]  # пропускаме първите n (най-малките числа)

numbers.sort(reverse=True)
numbers = list(map(str, numbers))
result = ", ".join(numbers)
print(result)

"""


"""
6.	Survival of the Biggest
Write a program that receives a list of integer numbers (separated by a single space) and a number n. 
The number n represents the count of numbers to remove from the list. 
You should remove the smallest ones, and then, you should print all the numbers that are left in the list,
separated by a comma and a space ", ".

Example
Input	                Output
10 9 8 7 6 5
3	                10, 9, 8
1 10 2 9 3 8
2	                10, 9, 3, 8


"""

# По-бързи методи на решаване относно производителност:
"""
# Сортиране и изрязване
numbers.sort()
numbers = numbers[n:]  # пропускаме първите n (най-малките числа)

# С list comprehension
numbers = [x for x in numbers if x not in sorted(numbers)[:n]]
"""